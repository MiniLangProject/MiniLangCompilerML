...
function emit_value_to_string_function(state)
  ...
  // Format directly into the newly allocated string. The former path called
  // fn_int_to_dec(), whose process-global intbuf could be overwritten by a
  // concurrent native thread between conversion and the following copy.
  state.asm = a.mark(state.asm, l_int)
  int_count_nonzero = "v2s_int_count_nonzero_" + lid
  int_count_loop = "v2s_int_count_loop_" + lid
  int_count_done = "v2s_int_count_done_" + lid
  int_digits_positive = "v2s_int_digits_positive_" + lid
  int_digits_loop = "v2s_int_digits_loop_" + lid
  int_digits_done = "v2s_int_digits_done_" + lid
  ...
  state.asm = a.jcc(state.asm, "ge", int_count_nonzero)
  state.asm = a.neg_r64(state.asm, "r11")
  state.asm = a.inc_r32(state.asm, "r9d")
  state.asm = a.mark(state.asm, int_count_nonzero)
  state.asm = a.test_r64_r64(state.asm, "r11", "r11")
  state.asm = a.jcc(state.asm, "nz", int_count_loop)
  state.asm = a.inc_r32(state.asm, "r9d")
  state.asm = a.jmp(state.asm, int_count_done)
  state.asm = a.mark(state.asm, int_count_loop)
  state.asm = a.mov_r64_r64(state.asm, "rax", "r11")
  state.asm = a.xor_r32_r32(state.asm, "edx", "edx")
  state.asm = a.mov_r32_imm32(state.asm, "r8d", 10)
  state.asm = a.div_r64(state.asm, "r8")
  state.asm = a.mov_r64_r64(state.asm, "r11", "rax")
  state.asm = a.inc_r32(state.asm, "r9d")
  state.asm = a.test_r64_r64(state.asm, "r11", "r11")
  state.asm = a.jcc(state.asm, "nz", int_count_loop)
  state.asm = a.mark(state.asm, int_count_done)
  state.asm = a.mov_membase_disp_r32(state.asm, "rsp", 0x28, "r9d")

  // Allocate the object header, payload and trailing NUL.
  state.asm = a.mov_r32_r32(state.asm, "ecx", "r9d")
  state.asm = a.add_r32_imm(state.asm, "ecx", 9)
  state.asm = a.call(state.asm, "fn_alloc")

  // Restore the tagged input and length after the allocator's volatile clobbers.
  state.asm = a.mov_r64_membase_disp(state.asm, "r10", "rsp", 0x20)
  state.asm = a.sar_r64_imm8(state.asm, "r10", 3)
  state.asm = a.mov_r32_membase_disp(state.asm, "r9d", "rsp", 0x28)
  ...
  state.asm = a.jcc(state.asm, "nz", int_digits_positive)
  state.asm = a.dec_r64(state.asm, "r11")
  state.asm = a.mov_membase_disp_imm8(state.asm, "r11", 0, 0x30)
  state.asm = a.jmp(state.asm, int_digits_done)
  state.asm = a.mark(state.asm, int_digits_positive)
  state.asm = a.jcc(state.asm, "ge", int_digits_loop)
  state.asm = a.mov_r64_membase_disp(state.asm, "rax", "rsp", 0x20)
  state.asm = a.mov_membase_disp_imm8(state.asm, "rax", 8, 0x2D)
  state.asm = a.neg_r64(state.asm, "r10")
  state.asm = a.mark(state.asm, int_digits_loop)
  ...
  state.asm = a.dec_r64(state.asm, "r11")
  state.asm = a.mov_membase_disp_r8(state.asm, "r11", 0, "dl")
  state.asm = a.mov_r64_r64(state.asm, "r10", "rax")
  state.asm = a.test_r64_r64(state.asm, "r10", "r10")
  state.asm = a.jcc(state.asm, "nz", int_digits_loop)
  state.asm = a.mark(state.asm, int_digits_done)
  state.asm = a.mov_r64_membase_disp(state.asm, "rax", "rsp", 0x20)
  state.asm = a.jmp(state.asm, l_done)
  ...
  // Allocate first and let _gcvt write into the owned string payload. This
  // removes the equivalent cross-thread race through process-global floatbuf.
  state.asm = a.movsd_membase_disp_xmm(state.asm, "rsp", 0x20, "xmm0")
  state.asm = a.mov_rcx_imm32(state.asm, 72)
  state.asm = a.call(state.asm, "fn_alloc")
  state.asm = a.mov_membase_disp_r64(state.asm, "rsp", 0x28, "rax")
  state.asm = a.movsd_xmm_membase_disp(state.asm, "xmm0", "rsp", 0x20)
...
function emit_string_add_function(state)
  ...
  // Reject actual void values before conversion. Pooled string literals such
  // as "void" may intentionally share obj_void's address and remain strings.
  state.asm = a.cmp_r64_imm(state.asm, "rcx", c.TAG_VOID)
  ...
  // Reject the unsupported sentinel only when conversion produced it; an
  // already-string "<unsupported>" value is ordinary user data.
