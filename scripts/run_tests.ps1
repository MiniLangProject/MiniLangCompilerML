...
function Invoke-CompilerVersionCheck {
  ...
  Write-LogLine "== $Name =="
  $timer = [System.Diagnostics.Stopwatch]::StartNew()
  $expected = "MiniLang Compiler 1.2.10"
  $exitCode = 0
...
try {
  ...
  if ($null -ne (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
    ...
    $linuxCases = @(
      ...
      [pscustomobject]@{ Name = "Linux compression codecs"; Source = "compression_codecs.ml"; RunArgs = @() },
  ...
  $nativePrimitiveCases = @(
    ...
    [pscustomobject]@{ Name = "portable compression codecs"; Source = "compression_codecs.ml" },
