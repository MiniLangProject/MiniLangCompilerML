...
if ([string]::IsNullOrWhiteSpace($Compiler)) { $Compiler = Join-Path $Root "build\mlc_win64.exe" }
...
& $Compiler (Join-Path $Root "tools\mltest.ml") $DiscoveryTool -I $Root
...
& $Compiler $GeneratedRunner $TestExecutable -I $Root
