...
compiler="${MINILANG_COMPILER:-$root/build/mlc_linux_x64}"
test_root="${1:-$root/tests/mltest_fixture}"
if [[ $# -gt 0 ]]; then shift; fi
artifacts="${MLTEST_ARTIFACTS:-$root/build/mltest-linux}"
mkdir -p "$artifacts"

"$compiler" "$root/tools/mltest.ml" "$artifacts/mltest" -I "$root" --target linux-x64
chmod +x "$artifacts/mltest"
"$artifacts/mltest" generate "$test_root" "$artifacts/runner.ml"
"$compiler" "$artifacts/runner.ml" "$artifacts/tests" -I "$root" --target linux-x64
