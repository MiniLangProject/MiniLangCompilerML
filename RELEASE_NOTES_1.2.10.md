...
Both CLI version switches and `MINILANG_VERSION` report 1.2.10. Ready-to-run
Windows x64 and Linux x64 compiler packages include `std/`, the target's native
video runtime, a quick-start example, build metadata and SHA-256 sidecars.
Extract the whole package and run `mlc.exe` on Windows or `./mlc` on Linux; no
